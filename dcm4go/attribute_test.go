...
func TestAttributeToString(t *testing.T) {
	...
	if err := testStringEquals("{tag:0x00020000,vr:UL,val:[4]}", attribute.String()); err != nil {
		t.Error(err)
	}
}
...
func TestAttributeAsLongWrongType(t *testing.T) {
	attribute := &attribute{FileMetaInformationGroupLengthTag, "UL", []uint16{0x04}}
...
func TestAttributeAsString(t *testing.T) {
	attribute := &attribute{ModalityTag, "CS", []string{"CT"}}
...
func TestAttributeAsStringWrongType(t *testing.T) {
	attribute := &attribute{ModalityTag, "CS", []byte{0x01}}

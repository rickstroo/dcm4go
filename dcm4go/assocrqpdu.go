...
func readAssocRQPDU(reader io.Reader) (*AssocRQPDU, error) {
	...
	for {
		...
		} else if itemType == acPresContextItemType || itemType == rqPresContextItemType { // presentation context
...
func writeAssocRQPDU(writer io.Writer, assocRQPDU *AssocRQPDU) error {
	...
	// write pdu type
	if err := writeByte(writer, aAssociateRQPDU); err != nil {
		return err
	}
	...
	if err := writeVariableItems(byteWriter, assocRQPDU.appContextName, assocRQPDU.presContexts, rqPresContextItemType, assocRQPDU.userInfo); err != nil {
		return err
	}
